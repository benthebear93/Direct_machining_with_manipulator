...
void PCprocess::Cloudcb(const sensor_msgs::PointCloud2 msg){ //std_msgs::Int32 msg
  ...
  if(mbflag_save ==0)
  {
    ...
    pcl::io::savePCDFile (filepath + "/pcd_data/actual_pc.pcd", *temp_cloud);
    ...
    pcl::io::savePCDFile (filepath + "/pcd_data/changed_pc.pcd", pc_transformed);
    mbflag_save = 1;
  }
  else
  {
    pcl::io::loadPCDFile<pcl::PointXYZRGB> (filepath + "/pcd_data/changed_pc.pcd", *ptr_transformed);
    ...
    *ptr_filtered = *ptr_transformed;
    ...
    pcl::io::loadPCDFile<pcl::PointXYZRGB> (filepath + "/pcd_data/new_cluster6.pcd", *segmented_pc);
