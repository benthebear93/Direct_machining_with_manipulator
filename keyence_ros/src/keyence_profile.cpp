...
void pos_callback(const std_msgs::Float32::ConstPtr& msg)
{
    pos_y = msg->data;
    temp = pos_y;
}
...
int main(int argc, char**argv)
{
    ...
    ros::Subscriber posy_sub = nh.subscribe("posy", 100, pos_callback);
    ...
    bool init_flag = true;
        ...
        pub.publish(pc_msg);
        /*cout<< "cloud " << pc_msg->points[0] << endl;
        if(pos_y!=temp){
            values.push_back(point3d);
        }
        cout << "size :" << values.size() << endl;
        cout << values[0].x << endl;*/
    ros::spin();
    }
    else{
